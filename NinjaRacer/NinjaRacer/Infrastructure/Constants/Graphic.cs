namespace NinjaRacer.Infrastructure.Constants
{
    public class Graphic
    {
        public const int WindowWidth = 800;
        public const int WindowHeight = 600;

        public const int CarInitialPositionX = 385;
        public const int CarInitialPozitionY = 450;
        public const int BufferWidthLeft = 215;
        public const int BufferWidthRight = 485;
        public const int BufferHeight = 100;
        public const int LeftOutOfRoadPosition = 230;
        public const int RightOutOfRoadPosition = 480;
        public const int RoadMapSpeed = 20;
        public const int HealthBarCoordX = 620;
        public const int HealthBarCoordY = 100;
        public const int HealthBarBorderCoordX = HealthBarCoordX - 2;
        public const int HealthBarBorderCoordY = HealthBarCoordY - 2;
        public const int ScoreCoordX = 620;
        public const int ScoreCoordY = 50;
        public const int PlayerSpeedX = 620;
        public const int PlayerSpeedY = 500;
        public const int PlayerProgressPositionX = 9;
        public const int PlayerProgressPositionY = 530;

        public const string RootDirectory = "Content";
    }
}
